...
counts, paths and scan time are demonstration data. The example bundle paths do
not exist, so the expanded file-size readings are zero. These images do not
establish update availability, website coverage or compatibility for real products.

The presentation host is not included in the release application or source
snapshot. Keep screenshot captions explicit about example data whenever reused.
